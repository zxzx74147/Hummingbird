...
/**
 * 
 * Blob Cache使用的SQLite Database封装
 * 
 * @author liukaixuan@baidu.com
 */
public class BdBlobCachePerTableDBManager extends BdCacheBaseDBManager<byte[]> {

	public BdBlobCachePerTableDBManager(BdSQLiteHelper helper) {
		super(helper);
	}

	public String onNewNameSpaceCreated(String nameSpace) {
		String m_tableName = BdCacheService.CACHE_TABLE_PREFIX + "b" + Math.abs(nameSpace.hashCode());

		String sql1 = "CREATE TABLE IF NOT EXISTS "
				+ m_tableName
				+ "(m_key VARCHAR(64) PRIMARY KEY, saveTime bigint(21) default 0, lastHitTime bigint(21) default 0, timeToExpire bigint(21) default 0, m_value blob)";
